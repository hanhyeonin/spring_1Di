...
public class Day1Main {
	...
	public static void main(String[] args) {
		...
		// 1) dao 객체를 service 가 사용하게 합시다. : 방법 1) setter 메소드로 전달 2) 생성자로 전달  
		BuyService service = new BuyService();
		service.setDao(dao);
		
		// 2) service 객체를 controller 가 사용
		BuyController controller = new BuyController();		// 기본생성자 생성으로 인해 생성 오류 없어짐.
		controller.setService(service); 					// 2) setter로 전달
		controller.buy();
	...
	// 스프링에서는 1), 2) 에 해당하는 의존객체 설정을 자동으로 합니다. 1) 생성자 주입(커스텀)  2) setter 주입  3) 필드 주입
